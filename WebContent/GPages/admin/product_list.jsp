...
<script type="text/javascript">
	...
	$(function(){
		//一开始获取第一页的商品列表
		getProductListByIndex(1);
	});
	
	/*
		根据分页下标获取商品列表
	*/
	function getProductListByIndex(index){
		var url="${pageContext.request.contextPath}/getProductListByPage";
		...
		$.ajax({  
            ...
            success: function(data){ 
            	...
            	$.each(data.beanList, function (index,value){
            		...
            		if(index%2 == 0){
	            		...
	            		"<td class='center'>"+
	            		"<a href='${pageContext.request.contextPath}/showProduct?productId="+value.fid+"' title='查看商品详情'>"+
	            		"<img src='${pageContext.request.contextPath}/GPages/admin/images/icon_view.gif'/>"+
	            		"</a> "+
	            		"<a href='${pageContext.request.contextPath}/editProduct?productId="+value.fid+"' title='编辑'>"+
           ...
           /*  	pageCode	1
            	...
            	*/
            	...
            	{
            		pageIndex = i+1;
            		pageHtml += "<li><a style='margin-right:8px;' href='javascript:;' onclick='getProductListByIndex("+pageIndex+")'>"+pageIndex+"</a></li>";
            	}
	...
	function searchProductByLike(index){
		var $name = $("#search-productName").val();
		if($.trim($name) == '' || $name.length == 0){
			alert("请输入你要搜索的产品名字。。。。");
		}
		else{
			var url="${pageContext.request.contextPath}/searchProductByLike";
			var param={"name":$name,"index":index};
			var isDrop=["no","yes"];
			$.ajax({  
	            type: "POST",  
	            url: url,  
	            ...
	            success: function(data){ 
	            	...
	            	if(data.totalCount == 0){
	            		$("#tbody-productList").html("暂无含有<font style='color:red;'>"+$name+"</font>的商品！");
	            		$("#product-pageIndex").html("");
	            		return;
	            	}
	           ...
	           /*  	pageCode	1
	            	...
	            	*/
	            	...
	            	{
	            		pageIndex = i+1;
	            		pageHtml += "<li><a style='margin-right:8px;' href='javascript:;' onclick='searchProductByLike("+pageIndex+")'>"+pageIndex+"</a></li>";
	            	}
...
<body>
	<div class="wrap">
		...
		<div class="operate">
			<input type="text" class="textBox length-long" placeholder="输入产品名称..." id="search-productName"/> 
			<input type="button" value="查询" class="tdBtn" onclick="searchProductByLike(1)" />
		</div>
