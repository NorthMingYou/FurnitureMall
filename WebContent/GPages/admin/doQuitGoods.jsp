...
<body>
 <div class="wrap">
  <div class="page-title">
    <span class="modular fl"><i class="order"></i><em>订单编号：${forder.orderid}</em></span>
  </div>
  <dl class="orderDetail">
   ...
   <dd>
    <ul>
     <li>
      ...
      <span class="h-cont h-left">${forder.user.username}</span>
     </li>
     <li>
      <span class="h-cont h-right">收件人：</span>
      <span class="h-cont h-left">${forder.receiptPeople}</span>
     </li>
     <li>
      <span class="h-cont h-right">联系电话：</span>
      <span class="h-cont h-left">${forder.receiptTel}</span>
     </li>
     <li>
      <span class="h-cont h-right">联系地址：</span>
      <span class="h-cont h-left">${forder.receiptaddress}</span>
     </li>
     <li>
      <span class="h-cont h-right">订单状态：</span>
      <span id="colors" class="h-cont h-left">
      	<c:choose>  
		    <c:when test="${forder.state == '1'}">待发货</c:when>  
		    <c:when test="${forder.state == '0'}">未支付</c:when>  
		    <c:when test="${forder.state == '2'}">已发货</c:when>  
		    <c:when test="${forder.state == '3'}">已签收</c:when>  
		</c:choose> 
      </span>
     </li>
     <li>
      <span class="h-cont h-right">下单时间：</span>
      <span class="h-cont h-left">${forder.buydate}</span>
     </li>
   ...
   <dd>
    <table class="list-style">
     ...
     <c:forEach items= '${forder.orderDetails}'  var="orderDetail">   
	     <tr align="center">
     ...
     <tr>
      <td colspan="5">
       <div class="fr">
        <span style="font-size:40px;font-weight:bold;"><i style="color:red;">订单共计金额：￥</i><b>${forder.totalprice}</b></span>
       </div>
      </td>
     </tr>
    </table>
   ...
   <dd>
	  <div class="BatchOperation" align="right">
	   	<a class="btnStyle" href="${pageContext.request.contextPath}/doQuitGood">同意申请</a>
	  </div>
   </dd>
  </dl>
 </div>
